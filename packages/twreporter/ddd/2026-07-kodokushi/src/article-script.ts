...
type IndexedSpec = Spec & { index: number; source: Spec }
...
function mediaElement(
  ...
  if (EDITOR) {
    enableEditor(element, mask, scale, repaint)
  }
  return element
}

async function rewrapSection(
  section: string,
  blocks: HTMLElement[],
  specs: IndexedSpec[],
): Promise<void> {
  ...
  const reference =
    blocks.find(
      (block) => !block.matches('h1, h2, h3, h4, h5, h6, [role="heading"]'),
    ) ?? blocks[0]
  ...
  const paint = () => {
    ...
    lastKey = key
    const referenceData =
      blockData.find((block) => !block.heading) ?? blockData[0]
    const contentWidth =
      width - referenceData.pad.left - referenceData.pad.right
    const scale = contentWidth / BASE_WIDTH
    const output = document.createDocumentFragment()
    const active = new Map<
      Annotation,
      { spans: HTMLElement[]; lastY: number }
    >()
...
async function main(): Promise<void> {
  ...
  const grouped = illustrations.reduce<Record<string, IndexedSpec[]>>(
    (result, spec) => {
  ...
  await Promise.all(
    Object.entries(grouped).map(async ([section, specs]) => {
      ...
      if (!start || !end)
        return console.warn(`Missing ${section} start/end anchors`)
